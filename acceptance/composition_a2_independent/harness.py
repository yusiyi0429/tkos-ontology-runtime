"""A2 reporting and source-pinned process control, reusing audited A1 plumbing."""
...
class A2Harness(Harness):
    def start_api(self, source: Path, *, old=False, updates=None):
        ...
        process = self.spawn('a2-api', args, updates=updates)
        def started():
            if ready.exists():
                return json.loads(ready.read_text())
            if process.poll() is not None:
                raise AssertionError('A2 API exited before readiness; inspect redacted process log')
            return None
        info = wait(started)
...
class A2Book(CaseBook):
    ...
    def save(self, **extra):
        ...
        gate_names.update({'source_input_http', 'read_authority_rechecks', 'review_boundaries', 'source_expiry'})
        matrix_passed = passed == len(MATRIX) == 18 and done_checks == all_checks
        ...
        result = {**self.metadata, 'scope': 'runtime-a2-company-composition',
                  'started_at': self.started_at, 'updated_at': now(),
                  'matrix_passed': matrix_passed, 'a2_local_matrix_accepted': accepted,
                  'all_selected_scenarios_completed': scenarios_complete,
                  'runtime_accepted': False,
                  'legacy_regression': 'reported separately; never inferred from A2 matrix',
