...
def register_v02(h, source, f):
    """Use the real maintenance API; no direct insert of protocol success rows."""
    profile_path = ROOT / 'docs/contracts/method-profile-0.2.json'
    registry_path = ROOT / 'docs/runtime-method-registry-0.2.json'
    ...
    adapter.cli('method-profile-' + tag, [
        ...
        '--contract-file', str(ROOT / 'docs/contracts/tkos-method-0.2.md'),
    ], expected_exit=0)
    policy = {
        'default_protocol': 'tkos.method', 'default_contract_version': 'tkos.method/0.2',
    ...
    adapter.cli('method-registry-' + tag, [
        'set-registry', *common, '--protocol-id', 'tkos.method', '--contract-version', 'tkos.method/0.2',
        '--content-json', str(registry_path),
    ], expected_exit=0)
    return profile


def seed_v02(env, path, label):
    ...
    with psycopg.connect(env.values['MIGRATION_DATABASE_URL'], row_factory=dict_row) as conn:
        ...
        for domain in fixture['domains'].values():
            ...
            content = old['content']
            for action in ['m1a_activate_signal', 'm1a_archive_signal', 'm1a_create_direct_issue', 'm1a_publish_brief', 'm1a_confirm_brief']:
